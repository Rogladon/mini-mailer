...
async function generateReport(report, rows, copyNumbers) {
  ...
  const calculateRowHeight = (text, columnWidth) => {
    ...
    return Math.max(lineCount, 1) * 20;
  };
  report.forEach((r) => {
    const originalRow = rows.find((row2) => row2.__rowNumber === r.rowNumber);
    ...
    const orgHeight = calculateRowHeight(r.name, 30);
    const contactHeight = calculateRowHeight(r.contacts, 30);
    row.height = Math.max(orgHeight, contactHeight, 20);
  });
  const timestamp = formatDateForFileName();
  const file = path.join(electron.app.getPath("desktop"), `отчет_рассылки_${timestamp}.xlsx`);
...
const cfgPath = electron.app.isPackaged ? path.join(path.dirname(electron.app.getPath("exe")), "accounts.json") : path.join(electron.app.getAppPath(), "/resources/accounts.json");
